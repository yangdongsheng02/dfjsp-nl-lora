...
本实验建模的是 **动态柔性作业车间（Dynamic FJSP）重调度**：在初始计划执行中途发生扰动（急单到达），需对 **全车间的全部工序** 重新给出机器选择与时间安排。单条样本的 **问题规模** 如下（与 `data_generator.py` 及现网数据集一致）：

| 维度 | 规格 | 说明 |
|------|------|------|
| 机器数 | **4**（M1–M4） | 同一时刻每台机器最多加工一道工序 |
| 初始作业 | **6** 个（J1–J6） | 每作业 **3–5** 道序工序，**柔性**：每道工序可在 1–4 台候选机上加工，加工时间因机而异 |
| 急单作业 | **1–2** 个（J7、J8…） | 在初始计划约 **50%** 进度（`t ≈ initial_makespan / 2`）到达 |
| 总作业数 | **7–8** | 6 个初始 + 1–2 个急单 |
| **标签工序总数** | **23–37 条/样本**（中位数 **30**） | 输出为 **全局完整排程**，覆盖初始作业与急单的 **每一道工序**，而非仅「未完成片段」 |
| 单条标签字段 | 每工序 **5 元组** | 工件 J、工序 O、机器 M、开始时刻、加工时长 + 行末 **makespan** |
| 题干动态信息 | `t`、机台状态、完成/进行中统计 | 描述插队时刻现场；与标签「从 0 起的全局时间轴」形成语义张力（见警示章节） |
| 优化目标 | 最小化 **makespan** | 标签由 **OR-Tools CP-SAT** 求解（单实例限时 3s） |

**数据集实测规模**（`train_data.jsonl` / `test_data.jsonl`）：

| 统计量 | 训练集（200） | 测试集（50） |
|--------|---------------|--------------|
| 排程工序数 | 23–37，中位 **30** | 26–34，中位 **30** |
| 最优 makespan | 34–82，中位 **51** | 33–80，中位 **54** |
| 插队时刻 t | 12–34，中位 **20** | 11–30，中位 **20** |
| 单条 SFT 序列（含题干） | 约 **1180–1456 tokens** | 约 **1226–1394 tokens** |

因此，模型需从 **数千字中文题干**（含全部作业的机器可选集与动态状态）生成 **约 30 行结构化自然语言排程**，任务难度远高于「几台机、几个作业」的简略表述。

标签由 CP-SAT 最优/可行解转为自然语言列表；**监督信号是完整 schedule，不是单步决策或局部补丁。**
