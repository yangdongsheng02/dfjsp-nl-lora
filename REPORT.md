...
- 4 台机器（M1–M4）
- 6 个初始作业，每作业 3–5 道工序，工序可在多台机器上加工（加工时间因机而异）
- 在初始计划约 50% 进度处插入 1–2 个急单作业
- 需在满足工序先后与机器互斥前提下，重调度 **全部工序**，最小化 makespan
...
标签中 `schedule` 为 **全局时间轴从 0 起算** 的完整排程（约 29–33 道工序/样本）；题干中则强调 **当前时刻 t**（如 t=18/21/24）及机器实时状态。二者在语义上存在张力（许多工序的开始时刻小于 t）。
